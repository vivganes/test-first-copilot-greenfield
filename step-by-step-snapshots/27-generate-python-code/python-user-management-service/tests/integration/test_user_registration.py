...
class TestUserRegistration:
    def test_successful_registration(self, client, db_session):
        ...
        # DB assertions
        user = db_session.execute(text("SELECT * FROM User WHERE email=:email"), {"email": payload["email"]}).mappings().fetchone()
        assert user is not None
        assert user["name"] == payload["name"]
        assert user["emailVerified"] == 0
        assert user["status"] == "Active"
        # Role assertion
        role = db_session.execute(text("SELECT * FROM UserRole WHERE userId=:userId"), {"userId": user["id"]}).mappings().fetchone()
        assert role is not None
        assert role["role"] == "Customer"
        # Address assertion
        address = db_session.execute(text("SELECT * FROM Address WHERE userId=:userId AND type='Shipping'"), {"userId": user["id"]}).mappings().fetchone()
